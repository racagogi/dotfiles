...
myKeys conf@(XConfig {XMonad.modMask = modm}) =
  M.fromList $
    [ ((modm, xK_space), sendMessage NextLayout),
      ...
      ((modm .|. shiftMask, xK_p), spawn "brave"),
      ((modm .|. shiftMask, xK_M), spawn "pactl set-sink-mute @DEFAULT_SINK@ toggle"),
      ((modm .|. shiftMask, xK_h), spawn "pactl set-sink-volume @DEFAULT_SINK@ -5%"),
      ((modm .|. shiftMask, xK_l), spawn "pactl set-sink-volume @DEFAULT_SINK@ +5%"),
